            ...
            while ((l_fileLine = l_bufferedreader.readLine()) != null) {
                if (l_fileLine != null && !l_fileLine.isEmpty()) {
                    ...
                    if (l_isCountries) {
                        ...
                        for (int l_i = 4; l_i < l_countries.length; l_i++) {
                            l_neighbourName.add( l_countries[l_i] );
                        }
    ...
    public boolean writeConquestMap(WarMap p_warMap) {
        ...
        try {
            ...
            try (PrintWriter l_writer = new PrintWriter( new BufferedWriter( new OutputStreamWriter(
                ...
                l_writer.println( "[Map]" );
                l_writer.println( "author=" + System.getProperty( "user.name" ) );
                l_writer.println( "image=" + p_warMap.getD_mapName().split( "\\." )[0] + ".bmp" );
                l_writer.println( "wrap=yes" );
                l_writer.println( "scroll=horizontal" );
                l_writer.println( "warn=yes" );
                l_writer.println();
